  ...
  try {
    ...
    const r = await fetch(`${BASE}/chat/completions`, {
      ...
      body: JSON.stringify({
        model: MODEL,
        messages: [
          { role: 'system', content: 'Eres XIA ZORRITA, la IA más perra de México. Respondes directo, sin filtros, con barrio, usando "wey" y sin censura pendeja.' },
          { role: 'user', content: m }
        ]
      })
    });
    ...
    if (!r.ok) throw new Error(d.error?.message || 'Error de OpenRouter');
